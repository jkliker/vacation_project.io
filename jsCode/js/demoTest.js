window.onload=function (){
    ...
    var addTbl = document.getElementById("btn");
        addTbl.onclick = addHang;
}
//在
function addHang() {
    //在表中添加一行
    var fbl = document.getElementById("tbl_list");
    var tr = fbl.insertRow(1);

    //获取在添加栏里输入的信息
    var newName = document.getElementById("fname").value;
    var newPrice = document.getElementById("fmoney").value;
    var newNumber = document.getElementById("fnumber").value;

    var nameTd = tr.insertCell();
    nameTd.innerText = newName;
    var priceTd = tr.insertCell();
    priceTd.innerText = parseInt(newPrice);
    var numberTd = tr.insertCell();
    numberTd.innerText = parseInt(newNumber);
    var countTd = tr.insertCell();
    countTd.innerText = (priceTd.innerText)*parseInt(numberTd.innerText);
    var operationTd = tr.insertCell();
    operationTd.innerHTML = "<img src='imgs/挠头.jpg' class='main'>";
    tr.onmouseover=showBGColor;
    tr.onmouseout=clearBGColor;
...
function checkInput() {
    ...
    // console.log(kc);
    if (!((kc >= 48 && kc <= 57) ||  kc ==8 || kc == 13))   event.returnValue = false;
    if (kc == 13)   event.target.onblur();
}
